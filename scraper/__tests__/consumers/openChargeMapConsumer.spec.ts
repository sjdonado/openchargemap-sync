...
describe('openChargeMapConsumer', () => {
  ...
  const countriesCount = 3;

  const messages: ScraperMessage[] = [
    {
      id: snapshotId,
      country: referenceData.Countries[0],
      countriesCount,
    },
    {
      id: snapshotId,
  ...
  it('should store POI data and create + udpate + delete old snapshot', async () => {
    ...
    (
      mockRepository.collections.poiListSnapshots.findOneAndDelete as jest.Mock
    ).mockImplementation();
